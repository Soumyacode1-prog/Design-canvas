import { AuthAttempt, Session } from "./models.js";
import { hashToken, readCookie } from "./security.js";
...
export const limitAuthAttempts = async (req, res, next) => {
    try {
        const windowMs = 15 * 60 * 1000;
        const bucket = Math.floor(Date.now() / windowMs);
        const attempt = await AuthAttempt.findOneAndUpdate({ _id: hashToken(`${req.ip}:${bucket}`) }, { $inc: { count: 1 }, $setOnInsert: { expiresAt: new Date((bucket + 1) * windowMs) } }, { upsert: true, returnDocument: "after" });
        if (attempt.count > 30) {
