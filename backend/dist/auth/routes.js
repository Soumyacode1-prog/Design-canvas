...
import { requireAuth, limitAuthAttempts } from "./middleware.js";
import { ACCESS_MS, REFRESH_MS, clearTokens, hashPassword, hashToken, newToken, readCookie, setTokens, verifyPassword } from "./security.js";
const router = Router();
...
router.post("/signup", limitAuthAttempts, async (req, res, next) => {
    try {
        ...
        if (!parsed.success) {
            res.status(400).json({ message: "Enter a name, valid email, and a password of 12–128 characters." });
            return;
        }
        ...
        if (error.code === 11000) {
            res.status(409).json({ message: "Unable to create account with this email. Try logging in." });
            return;
        }
        next(error);
    }
...
router.post("/refresh", async (req, res, next) => {
    try {
        ...
        const session = await Session.findOneAndUpdate({ refreshHash: oldHash, revoked: false, expiresAt: { $gt: new Date() } }, { $set: { refreshHash: hashToken(refresh), accessHash: hashToken(access), accessExpiresAt: new Date(Date.now() + ACCESS_MS) }, $push: { usedRefreshHashes: oldHash } }, { returnDocument: "after" });
        if (!session) {
...
router.post("/logout", async (req, res, next) => {
    try {
        ...
        const matches = [];
        if (refresh)
            matches.push({ refreshHash: hashToken(refresh) }, { usedRefreshHashes: hashToken(refresh) });
        if (access)
            matches.push({ accessHash: hashToken(access) });
        if (matches.length)
            await Session.updateMany({ $or: matches }, { $set: { revoked: true } });
        clearTokens(res);
        res.status(204).send();
    }
    catch (error) {
